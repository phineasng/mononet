...
def MLP_train(config, checkpoint_dir=None, data_dir=None):
    ...
    # Define the model
    model = MLP(num_feature=config['num_features'], num_class=config['num_classes'])
...
def main(num_samples=10):
    ...
    config = {
        "lr": tune.loguniform(1e-4, 1e-1), # tune.quniform(0.001, 0.0014, 0.00005),# tune.choice([0.0007, 0.0005, 0.00055, 0.0009]),# tune.loguniform(1e-4, 1e-1),
        # tune.choice([0.00045, 0.0005, 0.00055, 0.0009]),
        "batch_size": tune.choice([20, 40, 50, 75, 90, 120]),
    ...
    result = tune.run(
        partial(MLP_train, data_dir=data_dir),
        config=config,
    ...
    best_trained_model = MLP(best_trial.config['num_features'], best_trial.config['num_classes'])

    best_trained_model.accuracy_hist = best_trial.last_result['accuracy_hist']
    best_trained_model.loss_hist = best_trial.last_result['loss_hist']
    torch.save(best_trained_model, out_file_nn)
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(prog='MLP', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('--load', type=bool, default=False, help='Specify if one should load or train a MLP')
    parser.add_argument('--in_file_NN', type=str,
                        default=f'/Users/{short_name}/Documents/GitHub/MonoNet_single_cell/mlp_saved.pickle')
    # --in_file_NN
